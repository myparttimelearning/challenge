...
public class ImageLoaderView extends ImageView {
    ...
    public void setImageUrl(final String imageUrl) {
        ...
        ImageLoaderView.this.loadImagePlaceHolder();
    ...
    private void loadImageFromUrl(final String imageUrl) {
        new Thread() {
            @Override public void run() {
                final Bitmap bitmap = ImageLoaderView.this.getFromCache(getFileNameFromUrl(imageUrl));
                if (bitmap != null) {
                    ImageLoaderView.this.loadBitmap(bitmap);
                } else {
                    if (isThereInternetConnection()) {
                        ...
                        imageDownloader.download(imageUrl, new ImageDownloader.Callback() {
                            @Override public void onImageDownloaded(Bitmap bitmap) {
                                ImageLoaderView.this.cacheBitmap(bitmap, getFileNameFromUrl(imageUrl));
                                ImageLoaderView.this.loadBitmap(bitmap);
                            }
    ...
    private void loadImagePlaceHolder() {
        if (this.imagePlaceHolderResId != -1) {
            ((Activity) getContext()).runOnUiThread(new Runnable() {
                @Override public void run() {
                    ImageLoaderView.this.setImageResource(
                            ImageLoaderView.this.imagePlaceHolderResId);
                }
            });
        }
    }
